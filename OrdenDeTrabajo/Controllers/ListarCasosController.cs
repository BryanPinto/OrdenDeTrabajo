...
namespace WebSolicitudes.Controllers
{
    public class ListarCasosController : Controller
    {
        ...
        [HttpPost]
        public string CasosPendientesAgenda(FormCollection collection)
        {
            string datosJSON = string.Empty;
            //string txtVacio = collection["txtVacio"];
            string IDUsuario = "";
            try
            {
                ...
                if (rows != null)
                {
                    foreach (XmlNode row in rows)
                    {
                        ...
                        fila.Add(@"<a href='" + Url.Action("AgendarCaso", "AgendarCaso", new { id = numCasoXML }) + @"' class='btn btn-default btn-md center-block'>Agendar</a>");
